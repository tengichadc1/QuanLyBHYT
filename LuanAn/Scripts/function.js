...
function UpdateStudentInfo() {
    ...
    if ($("#ddlTonGiao").val() == "" || $("#ddlTonGiao").val() == undefined) {
        Swal.fire("Vui lòng nhập dữ liệu", "Tôn giáo chưa được chọn!", "warning")
        return;
    }
    if ($("#txtDienThoai").val() == "" || $("#txtDienThoai").val() == undefined) {
        Swal.fire("Vui lòng nhập dữ liệu", "Điện thoại chưa được nhập!", "warning")
        return;
    }
    ...
    if ($("#txtNoiCap").val() == "" || $("#txtNoiCap").val() == undefined) {
        Swal.fire("Vui lòng nhập dữ liệu", "Nơi cấp CMND chưa được nhập!", "warning")
        return;
    }
    if ($("#txtNgayCap").val() == "" || $("#txtNgayCap").val() == undefined) {
        Swal.fire("Vui lòng nhập dữ liệu", "Ngày cấp CMND chưa được nhập!", "warning")
        return;
    }
    //--
    if ($("#ddlQuocGiaThuongTru").val() == "" || $("#ddlQuocGiaThuongTru").val() == undefined) {
        Swal.fire("Vui lòng nhập dữ liệu", "Quốc gia chưa được chọn!", "warning")
        return;
    }
    if ($("#ddlTinhThanhThuongTru").val() == "" || $("#ddlTinhThanhThuongTru").val() == undefined) {
        Swal.fire("Vui lòng nhập dữ liệu", "Tỉnh thành chưa được nhập!", "warning")
        return;
    }
    if ($("#ddlQuanHuyenThuongTru").val() == "" || $("#ddlQuanHuyenThuongTru").val() == undefined) {
        Swal.fire("Vui lòng nhập dữ liệu", "Quận huyện chưa được nhập!", "warning")
        return;
    }
    if ($("#ddlPhuongXaThuongTru").val() == "" || $("#ddlPhuongXaThuongTru").val() == undefined) {
        Swal.fire("Vui lòng nhập dữ liệu", "Phường xã chưa được nhập!", "warning")
        return;
    }
    if ($("#txtSoNhaThuongTru").val() == "" || $("#txtSoNhaThuongTru").val() == undefined) {
        Swal.fire("Vui lòng nhập dữ liệu", "Số nhà, tên đường chưa được nhập!", "warning")
        return;
    }
    //--
    ...
    var para = new Object();
    para.STK = $('#txtSTK').val();
    para.TenNganHang = $('#txtTenNganHang').val();
...
function Onblur(cuobj, max, obj) {
    ...
    //|| parseInt(valueee) > parseInt(max) 

    if (valueee == '' || isNaN(valueee) == true || parseInt(valueee) < 0) 
    {
        ...
        ShowAlert("Thông báo", "Điểm không hợp lệ, nhập số và không được vượt mức điểm. Hệ thống sẽ trả điểm đó về 0.");
        return false;
    }
    else
    {
    ...
    // Anh lợi nhờ nếu âm thì nhập gì cg đc | 07.11.2018
    //if (parseInt(max) > 0)
    //{
...
function StudentKTX_Ins() {
    ...
    $.ajax({
        ...
        async: true,
        dataType: 'text',
        data: { Phong: Phong, Lau: Lau, NgayVao: NgayVao, YearStudy: YearStudy, TermID: TermID },
        success: function (html) {
            if (html == "0") {
                $("#myModal").modal('hide');
                ShowAlert("Thông báo", "Đăng ký thành công")
...
function StudentNgoaiTru_Ins() {
    ...
    var PhuongXa = $("#txtPhuongXaThuongTru").val();
...
function StudentKTX_Upd() {
    ...
    $.ajax({
        ...
        async: true,
        dataType: 'text',
        data: { Phong: Phong, Lau: Lau, NgayVao: NgayVao, YearStudy: YearStudy, TermID: TermID, IDlog: IDlog },
        success: function (html) {
            if (html == "0") {
                $("#myModal").modal('hide');
