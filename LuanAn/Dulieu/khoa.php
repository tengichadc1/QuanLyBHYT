    ...
    <div class="" style="background:#fff;min-height:100vh; margin: 0px 1px">
            ...
            <table id="tb-add" style="width: 100%;">
                ...
                <tr class="add-r">
                    <td class="form-outline1" style="width: 100%">
                        <input type="text" class="form-control form1" id="itenkhoa" name="itenkhoa" placeholder=" ">
                        <label class="form-label" for="itenkhoa">Tên khoa</label>
                    </td>
                </tr>
                <tr class="">
        ...
        <form id="f-search" method="post" class="hidden3">
            ...
            <div class="bl-search">
                <select style="width: auto" id="field-search" name="field-search" onchange="changeInputType()"> 
                    <option value="IDKhoa">ID Khoa</option>
        ...
        <form method="post" enctype="multipart/form-data">
            ...
            if (isset($_POST['d-accept'])) {
                if (isset($_POST['cb-IDKhoa'])) {
                    $selectedValues = $_POST['cb-IDKhoa'];
                    foreach ($selectedValues as $idkhoa) {
                        mysqli_query($connect, "Delete from khoa where IDKhoa = " . $idkhoa);
                    }
                }
            }
            ...
            if (isset($_POST['btn-updates'])) {
                $oldIDKhoa = $_POST["ip-IDKhoahiden"];
                $newTenKhoa = $_POST["ip-TenKhoa"];

                for ($i = 0; $i < count($oldIDKhoa); $i++) {
                    $uquery = "update khoa SET TenKhoa = '$newTenKhoa[$i]'
                            WHERE IDKhoa = $oldIDKhoa[$i]";
                                        ...
                                        <tbody>
                                            ...
                                            if (isset($_POST["btn-find"])) {
                                                if ($_POST["ip-search"] == "null") {
                                                    ...
                                                    $selectKhoa = "Select * from khoa where $getfield = ' '";
                                                } elseif ($_POST["ip-search"] == "") {
                                                    header("location: /LuanAn/Dulieu/khoa.php");
                                                } else {
                                                    ...
                                                    $getfield = $_POST["field-search"];
                                                    $selectKhoa = "Select * from khoa where " . $getfield . " LIKE '%$getquery%' ";
                                                }
                                                ...
                                                if (mysqli_num_rows($dataKhoa) > 0) {
                                                    ...
                                                    while ($rows = mysqli_fetch_assoc($dataKhoa)) {
                                                        $IDKhoa = $rows["IDKhoa"];
                                                        $TenKhoa = $rows["TenKhoa"];

                                                        echo ("
                                                        <tr>
                                                            <td class='hidden1'><input type='checkbox' class='cb-IDLop' name='cb-IDKhoa[]' value='$IDKhoa'></td>
                                                            <td style='display: none'><input type='text' name='ip-IDKhoahiden[]' value='$IDKhoa'></td>
                                                            <td class='r-header' style='text-align: center; line-height: 30px'>$stt</td>
                                                            <td class='r-header' >
                                                                <input type='text' style='text-align: center; width: 100%' name='ip-TenKhoa[]' value='$TenKhoa'>
                                                            </td>                         
                                                        </tr>
                                                        ");
                                                        $stt++;
                                                ...
                                                $Khoa = mysqli_query($connect, "Select * from khoa");
                                                $selectKhoa = "Select * from khoa order by IDKhoa";
                                                $dataKhoa = mysqli_query($connect, $selectKhoa);
                                                if (mysqli_num_rows($dataKhoa) > 0) {
                                                    $stt = 1;
                                                    while ($rows = mysqli_fetch_assoc($dataKhoa)) {
...
?>
    ...
    <script type="text/javascript">
        ...
        document.getElementById('btn-add').addEventListener('click', function () {
            var itenkhoa = document.getElementById('itenkhoa').value;

            var xhr = new XMLHttpRequest();
            xhr.open('POST', 'insertKhoa.php', true);
            ...
            var formData = 'TenKhoa=' + encodeURIComponent(itenkhoa);
            xhr.send(formData);
